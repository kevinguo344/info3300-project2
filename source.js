...
function rank(){
	...
	var colorScale = d3.scaleLinear()
		.domain([200, 180, 160, 140, 120, 100, 80, 60, 40, 20, 1])
		.range(['#add8e6','#9dc0e5','#8da7e4','#7c90e1','#6b78dc','#5961d5','#464aca','#3333ba','#2120a0','#191970']);
...
function getinfo(item,time){
	var info = "";
	info += "<p><b><i>"+ item.album + "</i></b></p>";
	info += "<p>"+ item.artist[0]+ "</p>";
	if(item.top_chart_position!=100000)
		info += "<p>Top Position on Billboard 200: " + item.top_chart_position + "</p>";
...
function getArtist(item){
	...
	info += "<p><b>"+ item.artist + "</b></p>";
	info += "<p>Average Score: "+ item.avg_score.toFixed(2) + "</p>";
	info += "<p>Peak Position on Billboard Top 100: " + item.top_chart_position + "</p>";
	info += "<p>Albums on the Billboard Top 100: "+ item.num_top_charted + " of " + item.albums.length + "</p>";
...
function showpie(data){
	...
	var color = ['#2120a0','#464aca','#6b78dc','#8da7e4','#add8e6'].reverse();
	...
	svg_pie.append("text").attr("class","exposeText")
	 ...
	 svg_pie.append("text").attr("class","exposeText")
		.attr("x","10")
		.attr("y","184")
		.text("ended up on the Billboard Top 200.")
		.style("text-anchor", "left")
		.style("alignment-baseline","center")
